    ...
    if (!exception.check(entity, constraintData)) {
      ...
      let qs = {
        query:
          "ASK { { wd:" +
          entity +
          " ?p [] . ?property wikibase:statement ?p; p:P2302 ?constraint . ?constraint ps:P2302 wd:Q21502838; pq:P2306 wd:" +
          p +
          " . OPTIONAL { ?constraint pq:P2305 ?conflictValue  } FILTER(IF(BOUND(?conflictValue), ?conflictValue  = " +
          value +
          ", true))} " +
          check2 +
          "}",
