...
function parseMediawikiTemplate(t, templateprefixes, templateredirects, template) {
  ...
  let text = t
    ...
    .replace(new RegExp('{{\\s*(:?(' + templateprefixes.join('|') + '):\\s*)?(' + templateredirects.join('|') + ')\\s*', 'gi'), '{{' + template);
...
function harvester(req, res) {
  ...
  .then(response => {
      ...
      let params = parseMediawikiTemplate(response.data.query.pages[m.pageid].revisions[0].slots.main['*'], job.templateprefixes, job.templateredirects, job.template);
      if (params === null) {
      ...
      } else { //if multiple parameters need to be parsed, e.g. dates
